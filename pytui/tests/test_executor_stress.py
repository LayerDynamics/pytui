...
def test_large_output_handling(large_output_script):
    ...
    try:
        ...
        while time.time() - start_time < max_wait and not (all_stderr_received and all_stdout_received):
            # Check for last lines of output
            stdout_lines = [line.content for line in executor.collector.output if line.stream == "stdout"]
            stderr_lines = [line.content for line in executor.collector.output if line.stream == "stderr"]
...
def test_recursive_function_tracing(recursive_function_script):
    ...
    try:
        ...
        while time.time() - start_time < max_wait:
            if any("Final result: 120" in line.content for line in executor.collector.output):
                break
            time.sleep(0.1)
            
        # Check that execution completed
        assert any("Final result: 120" in line.content for line in executor.collector.output), \
            "Script did not complete successfully"
            
        # Check function calls were traced - there should be multiple recursive_func calls
        recursive_calls = [call for call in executor.collector.calls 
        ...
        return_events = len([ret for ret in executor.collector.returns 
...
""")
        ...
        while time.time() - start_time < max_wait and not all_completed:
            ...
            all_completed = all(
                any(f"Script {i} completed" in line.content 
        ...
        for i, executor in enumerate(executors):
            assert any(f"Script {i} completed" in line.content 
                      for line in executor.collector.output), \
        ...
        for i, executor in enumerate(executors):
            ...
            for j in range(5):
                if j != i:
                    assert not any(f"Script {j} " in line.content 
                                  for line in executor.collector.output), \
...
""")
    ...
    try:
        ...
        assert any("ValueError" in line.content for line in executor.collector.output), \
