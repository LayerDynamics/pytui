...
# Replace third-party imports with our rich_text_ui components
...
# Local imports
from .widget_utils import ActiveFunctionHighlighter, SearchableText
from .widgets import AnimatedSpinnerWidget, VariableInspectorWidget
...
# Define ROUNDED constant
ROUNDED = Box.get_box_style("rounded")


# Tree implementation using our container
class Tree(Container):
    ...
    def render(self):
        """Render the tree."""
        return self._render_node(self.label, list(self.children), 0)

    def _render_node(self, label, children, level):
        ...
        for child in children:
            if hasattr(child, "render"):
                ...
                child_content = str(child)

            if hasattr(child, "children") and child.children:
                child_result = self._render_node(child.label, child.children, level + 1)
                result.append(child_result)
            else:
                result.append(f"{indent}  {child_content}")
...
class BasePanel(Container):
    ...
    async def refresh_content(self):
        ...
        if self._console:
            layout = Layout()
            layout.split_column(
                Layout(title=self.title), Layout(renderable="Content goes here")
            )
...
class OutputPanel(BasePanel, ScrollView):
    ...
    def _prepare_renderables(self) -> List[Any]:
        ...
        if self.search_mode and self.search.match_indices:
            ...
            renderables = self.search.highlight_matches(
    ...
    def _filter_logs(self) -> List[Dict[str, Any]]:
        ...
        if self.filter_text:
            ...
            filtered_logs = [
                ...
                if (
                    filter_text in log["message"].lower()
