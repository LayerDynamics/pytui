...
def _send_trace_event(event_type: str, func_data: Dict[str, Any]):
    ...
    try:
        ...
        # For critical events, ensure they're written
        if event_type == "call" and func_data.get("function_name") == "function1":
...
def _get_call_id() -> int:
    ...
    global _last_call_id
    _last_call_id += 1
    return _last_call_id


def _get_parent_id() -> int:
    """Get parent call ID from stack."""
    return _current_call_stack[-1] if _current_call_stack else None
...
def _record_trace_event(event: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if _trace_file and not _trace_file.closed:
            with open(_trace_file.name, 'a', encoding='utf-8') as f:
                f.write(event_json + '\n')
                f.flush()
    except Exception:
        pass
