...
try:
    ...
    # Instead of silent mocking, raise a clear error message about missing dependencies
    import sys
    print(f"ERROR: Required dependency 'textual' is not available: {e}", file=sys.stderr)
    print("Please install the required dependencies:", file=sys.stderr)
    print("    pip install textual>=0.9.0", file=sys.stderr)
    print("    # or", file=sys.stderr)
    print("    conda install -c conda-forge textual", file=sys.stderr)
    sys.exit(1)
