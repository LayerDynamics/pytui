...
def handle_signal(signum, frame):
    print("Received signal to terminate", flush=True)
    sys.exit(0)
...
def memory_error_simulation():
    ...
    try:
        big_list = [0] * (10**6)  # Reduced size for faster execution
...
def test_error_handling(error_script):
    ...
    try:
        ...
        expected_messages = [
            "Starting error script",
            "About to cause a file error",
            "Caught file error",
            "About to cause a runtime error",
        ...
        while time.time() - start_time < max_wait:
            if not executor.is_running and executor.process.poll() is not None:
                break

            current_output = [line.content for line in executor.collector.output]
            for message in expected_messages:
                if any(message in line for line in current_output):
        ...
        assert not missing_messages, f"Missing expected output: {missing_messages}"
        assert executor.process.poll() in (0, None), "Process failed unexpectedly"
