...
def test_error_handling(error_script):
    ...
    try:
        ...
        while time.time() - start_time < max_wait and not completed:
            if any("Script completed successfully" in line.content for line in executor.collector.output):
                completed = True
            time.sleep(0.1)
        ...
        assert "Simulating memory pressure" in outputs, "Memory error simulation not run"
...
def test_long_running_script_termination(long_running_script):
    ...
    try:
        ...
        while time.time() - start_time < max_wait and not started:
            if any("Still running" in line.content for line in executor.collector.output):
                started = True
            time.sleep(0.1)
        ...
        assert len(executor.collector.output) == output_count, "Output continued after termination"
    finally:
        # Ensure cleanup
        if executor:
            executor.stop()
...
def test_stdout_flood_handling(stdout_flood_script):
    ...
    try:
        ...
        while time.time() - start_time < max_wait:
            if any("Finished flooding stdout" in line.content for line in executor.collector.output):
                break
            time.sleep(0.1)
        ...
        assert not executor.is_running or executor.process.poll() is not None, \
...
def test_tracer_exception_handling():
    ...
    assert "unable to represent" in prob_calls[0].args["bad_repr_obj"], \
