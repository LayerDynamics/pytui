...
class TraceState:
    ...
    @property
    def trace_file(self): return self.files.trace_file
    @trace_file.setter
    def trace_file(self, value): self.files.trace_file = value

    @property
    def current_trace_file(self): return self.files.current_trace_file
    @current_trace_file.setter
    def current_trace_file(self, value): self.files.current_trace_file = value

    @property
    def collector(self): return self.collectors.collector
    @collector.setter
    def collector(self, value): self.collectors.collector = value

    @property
    def current_collector(self): return self.collectors.current_collector
    @current_collector.setter
    def current_collector(self, value): self.collectors.current_collector = value

    @property
    def current_call_stack(self): return self.calls.current_call_stack
    @current_call_stack.setter
    def current_call_stack(self, value): self.calls.current_call_stack = value

    def get_collector(self): return self.collectors.get_collector()
    def set_collector(self, collector): self.collectors.set_collector(collector)
    def reset(self): self.calls.reset()
...
def _handle_call_event(frame, func_name, filename, collector):
    ...
    parent_id = _state.calls.current_call_stack[-1] if _state.calls.current_call_stack else None
    _state.calls.current_call_stack.append(call_id)

    collector.add_call(
        func_name, filename, line_no, args_dict,
...
def _get_parent_id() -> int:
    ...
    return _state.calls.current_call_stack[-1] if _state.calls.current_call_stack else None
...
def _trace_call(frame, event, arg) -> None:
    ...
    try:
        if event == "call":
            _handle_call_event(frame, frame.f_code.co_name, 
                             frame.f_code.co_filename, _state.get_collector())
            return _trace_call
        if event == "return":
            _handle_return_event(frame, arg, frame.f_code.co_name, 
