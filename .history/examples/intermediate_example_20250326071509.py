...
class TaskList(UIList):
    ...
    def update_tasks(self, tasks: List[Any]) -> None:
        ...
        for task in tasks:
            ...
            self.add_item(
                f"Task {task.id}: {task.name} - Priority: {task.priority} "
                f"[Status: {task.status}]",
                color=status_color
            )
    
    def get_filtered_tasks(self) -> List[Any]:
...
class DataViewer:
    ...
    def build_ui(self):
        ...
        text_container = VBox()
        stats_text = Text()
        stats_text.set_text("Statistics")
        stats_text.set_color(Color.CYAN)
        help_text = Text()
        help_text.set_text("Use the filter above to search data")
        help_text.set_color(Color.BLUE)
    ...
    def update_table(self, filter_text=""):
        ...
        filtered_data = [
            row for row in self.data if filter_text.lower() in str(row[1]).lower()
        ]
        ...
        self.status_text.update(
            f"Showing {len(filtered_data)} of {len(self.data)} items",
