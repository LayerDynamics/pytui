...
class PyTUIApp(App):
    ...
    async def on_mount(self):
        ...
        self.spinner = AnimatedSpinnerWidget("Running script...", "dots3", show_elapsed=True)
        ...
        while self.executor.is_running:
            try:
                ...
                if event_type == 'output':
                    ...
                    if event.args:
                        self.variable_inspector.set_variables(event.function_name, event.args)
                    self.timeline_widget.add_event("call", f"{event.function_name}() at {event.filename}:{event.line_no}")
                    # Update spinner text
                    self.spinner.set_text(f"Executing {event.function_name}()")
                elif event_type == 'return':
                    await self.call_graph_panel.add_return(event)
                    self.timeline_widget.add_event("return", f"{event.function_name}() → {event.return_value}")
                elif event_type == 'exception':
                    await self.exception_panel.add_exception(event)
                    await self.output_panel.add_exception(event)
                    self.timeline_widget.add_event("exception", f"{event.exception_type}: {event.message}")
                    # Update status for exception
                    self.status_bar.set_status_message(f"Exception: {event.exception_type}")
            except asyncio.CancelledError:
                break
            except Exception as e:
    ...
    async def action_toggle_collapse(self):
        ...
        if focused == self.call_graph_panel:
            self.collapsed_panels['call_graph'] = not self.collapsed_panels['call_graph']
