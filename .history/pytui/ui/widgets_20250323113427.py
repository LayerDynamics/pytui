...
from rich.spinner import Spinner"""Status bar for displaying execution info."""
from rich.panel import Panel

class StatusBar(Static):atus bar."""
    """Status bar for displaying execution info."""
    
    def __init__(self): to regular attributes
        """Initialize the status bar."""e
        super().__init__("")self._is_paused = False
        self.start_time = time.time()
        # Change from properties to regular attributespe:
        self._is_running = False."""
        self._is_paused = False
        
    def render(self) -> RenderableType:elapsed_str = f"{int(elapsed // 60):02d}:{int(elapsed % 60):02d}"
        ...
        # Determine status
        if self._is_paused:
            status = "PAUSED"tatus_style = "bold green"
            status_style = "bold yellow"
        elif self._is_running:
            status = "RUNNING"status_style = "bold red"
        ...
        text.append("Elapsed: ", style="bold")old")
        text.append(elapsed_str, style="cyan"))
        text.append(" | ", style="dim")text.append("[/]Search", style="bold")
        text.append("[Q]uit ", style="bold")
        text.append("[P]ause/Resume ", style="bold")return text
        text.append("[R]estart ", style="bold")
        text.append("[/]Search", style="bold"): bool):
        
        return textng = is_running
        self.refresh()
    def set_running(self, is_running: bool):
        """Set the running state.""" bool):
        self._is_running = is_running
        self.refresh()d = is_paused
        self.refresh()
    def set_paused(self, is_paused: bool):
        """Set the paused state."""
        self._is_paused = is_pausednter."""
        self.refresh()e = time.time()
            self.refresh()
    def reset_timer(self):
        """Reset the elapsed time counter."""perties for test compatibility
        self.start_time = time.time()
        self.refresh()
    ...
    @property
    def is_running(self):
        return self._is_running        return self._is_paused
