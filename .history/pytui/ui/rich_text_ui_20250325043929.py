...
class StyleManager:
    ...
    async def animate(self, property_name, target_value, duration=0.3, easing="linear"):
        ...
        # Validate easing
        valid_easings = {"linear", "ease-in", "ease-out"}
        if easing not in valid_easings:
            easing = "linear"

        try:
            # Get current value, defaulting to 0 if not set
            start_value = float(self._styles.get(property_name, 0))
            target_value = float(target_value)
            
            # Calculate total steps based on 60fps and duration
            total_steps = int(duration * 60)
            if total_steps < 1:
                # For instant changes
            ...
            ease_func = easing_funcs.get(easing, linear)
            ...
            for step in range(total_steps + 1):
                ...
                current = start_value + (target_value - start_value) * eased_progress
            ...
            # Fallback for non-numeric values or errors
            self._styles[property_name] = target_value
            await self.widget.refresh()
...
class Panel(Widget):
    ...
    def render(self):
        ...
        for line in content_lines:
            ...
            padded_line = line + " " * max(
                0, width - 2 - len(line) - self.padding[0] * 2
            )
            lines.append(
                v + " " * self.padding[0] + padded_line + " " * self.padding[0] + v
            )
