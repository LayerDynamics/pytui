...
class Box:
    ...
    def create_header(width: int, style="rounded", text="") -> str:
        ...
        if text:
            ...
            left_pad = padding // 2
            right_pad = padding - left_pad
            return (box_chars["tl"] + box_chars["h"] * left_pad + " " + 
                   text + " " + box_chars["h"] * right_pad + box_chars["tr"])
...
class Table(Widget):
    ...
    def add_row(self, *values, style=None):
        ...
        for i, value in enumerate(values):
            ...
            if i < len(self._data.column_widths):
                self._data.column_widths[i] = max(self._data.column_widths[i], len(value_str))
    ...
    def render(self):
        ...
        box_chars = {k: self.box.get(k, v) for k, v in {
            "tl": "┌", "tr": "┐", "bl": "└", "br": "┘", "h": "─", "v": "│"
        }.items()}
        ...
        # Add bottom border
        lines.append(box_chars["bl"] + box_chars["h"] * len(separator) + box_chars["br"])
