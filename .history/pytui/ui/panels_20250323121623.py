...
from ..utils import truncate_string, format_time
...
class OutputPanel(BasePanel, ScrollView):
    ...
    async def add_message(self, message: str):
        ...
        text.original_output = OutputLine(content=message, stream="system")
        self.outputs.append(text)
        await self.update_content()
    ...
    async def add_exception(self, exc: ExceptionEvent):
        ...
        text.original_output = OutputLine(
            content=f"Exception: {exc.exception_type}: {exc.message}",
            stream="exception",
        )
        self.outputs.append(text)
        await self.update_content()
    ...
    async def update_content(self):
        ...
        if self.search_mode and self.search.match_indices:
            ...
            highlighted = self.search.highlight_matches(
        ...
        if self.search_mode:
            ...
            if self.search.match_indices:
                ...
                search_text.append(
                    f" ({current}/{len(self.search.match_indices)} matches)"
                )
        ...
        if self.show_timestamps:
            for i, renderable in enumerate(renderables):
                if hasattr(renderable, "original_output") and hasattr(
                    renderable.original_output, "timestamp"
                ):
        ...
        if self.group_by_stream:
            ...
            for renderable in renderables:
                if hasattr(renderable, "original_output") and hasattr(
                    renderable.original_output, "stream"
                ):
            ...
            content = (
                Columns(group_displays)
                if group_displays
                else Panel("No output", title="Output")
            )
        else:
            # Regular display
    ...
    def _filter_outputs(self):
        ...
        return [
            ...
            if (
                hasattr(out, "original_output")
                and filter_text in out.original_output.content.lower()
            )
            or filter_text in str(out).lower()
        ]
...
class CallGraphPanel(BasePanel):
    ...
    async def on_mount(self):
        ...
        try:
            ...
            # Add variable inspector but hide it initially
            self.variable_inspector._parent = self
...
class LogPanel(BasePanel, ScrollView):
    ...
    async def update_content(self):
        ...
        filtered_logs = [
            log for log in self.logs if log["level"] in self.visible_levels
        ]
        ...
        if self.filter_text:
            ...
            filtered_logs = [
                ...
                or filter_text in log["source"].lower()
            ]
