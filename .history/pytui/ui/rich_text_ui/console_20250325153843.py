...
class ConsoleBase(Widget):
    ...
    def write(
        ...
        event = self._emit_event(
            "write", {"text": text, "style": style, "timestamp": timestamp, "level": level}
        )
...
class Console(ConsoleBase):
    ...
    _action_handlers = {
        ...
        'next': lambda self: self._handle_search("next"),
        'prev': lambda self: self._handle_search("prev"),
        'export': lambda self, filename, fmt='text': self._handle_export(filename, fmt)
    }
    ...
    def _handle_export(self, filename: str, fmt: str = "text") -> bool:
        """Internal method to handle export operations."""
        return self.export(filename, fmt)
