...
class ConsoleLogging:
    ...
    async def write(self, text: str, style: Optional[Style] = None, level: Optional[str] = None) -> None:
        """Write text to console."""
        raise NotImplementedError("Subclass must implement write() method")

    async def refresh(self) -> None:
        """Refresh console display."""
        raise NotImplementedError("Subclass must implement refresh() method")
    ...
    async def log(self, *messages: Union[str, Text], sep: str = " ",
        ...
        await self.write(text, style=style, level=level)
        await self.refresh()

    async def log_message(self, level: LogLevel, *messages: Union[str, Text],
            sep: str = " ", style: Optional[Union[str, Style]] = None) -> None:
        """Log message with specified level."""
        text = sep.join(str(m) for m in messages)
        await self.write(text, style=style, level=level.name)
        await self.refresh()
