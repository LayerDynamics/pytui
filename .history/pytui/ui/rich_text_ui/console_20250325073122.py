...
class ConsoleConfig:
    ...
    log_level: str = "INFO"
    highlight_rules: Optional[Dict[str, str]] = field(default_factory=dict)
...
class Console(Widget):
    ...
    # Properties for public interface
    @property
    ...
    def max_lines(self) -> int:
        """Get maximum number of lines to keep."""
    ...
    def default_style(self) -> Optional[Style]:
        ...
        return Style(self._config.style) if self._config.style else None
        ...
        if self._config.log_file:
            try:
                ...
                with open(self._config.log_file, "a", encoding="utf-8") as f:
                    f.write("--- Console log started ---\n")

                # Then open a persistent handle
                self._state.log_file_handle = contextlib.closing(
                    open(self._config.log_file, "a", encoding="utf-8")
                ).__enter__()
