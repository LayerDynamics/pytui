...
class Console(Widget):
    ...
    def __init__(
        ...
        *,
        ...
        self._buffer: List[tuple[str, Optional[Style], int]] = (
            []
    ...
    async def stop_spinner(
        ...
        if self._spinner_task and not self._spinner_task.done():
            ...
            if self._active_spinner is not None and self._active_spinner < len(
                self._buffer
            ):
                ...
                final_style = (
                    Style(style) if style else self._buffer[self._active_spinner][1]
                )
