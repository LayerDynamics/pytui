...
class Console(Widget):
    ...
    def _apply_highlighting(self, content: str) -> str:
        ...
        for pattern, highlight_style in self._config.highlight_rules.items():
            ...
            try:
                content = re.sub(
                    pattern,
                    lambda m: Style(style_str).apply(m.group(0)),
                    content
                )
    ...
    def _highlight_search_match(self, index: int, line: str) -> str:
        ...
        if (current_idx >= 0 and 
            current_idx < len(self._search_state["results"]) and
            index == self._search_state["results"][current_idx]):
    ...
    def next_result(self) -> int:
        ...
        self._search_state["index"] = (self._search_state["index"] + 1) % len(self._search_state["results"])
        self._dirty = True
    ...
    async def start_spinner(
        ...
        spinner_chars = self._spinner_state["chars"].get(spinner_type, self._spinner_state["chars"]["dots"])
