...
class ConsoleBase(Widget):
    ...
    def __init__(self):
        ...
        super().__init__()
        self._emit_event = Event("console_init")
    ...
    def write(
        ...
        self._emit_event.prevent_default()  # Use the event
        
        # Process text with style
        content = str(text)
        if style:
            content = Style(style).apply(content) if isinstance(style, str) else style.apply(content)
