...
from typing import Optional, List, Union, TextIO, Dict, Any, Tuple
...
from .events import Event, KeyEvent  # KeyEvent used for type hints
...
class ConsoleBase(Widget):
    ...
    def __init__(self):
        ...
        super().__init__()
        self._emit_event = Event("console_init")  # Use Event class
        self._write_handlers = []
        self._state = ConsoleState()  # Initialize state in base class

    def write(self, text: Union[str, Text], style=None, timestamp=None, level=None) -> "ConsoleBase":
        """Write content to console with optional styling."""
        # Implement write in base class
        return self
...
class Console(ConsoleBase):
    ...
    def _handle_search(self, action: str, text: str = "") -> Union[List[int], int]:
    ...
    def progress(self, total: int = 100, description: str = ""):
        ...
        Returns:
            Generator yielding a ProgressBar instance
        """
    ...
    def search(self, text: str) -> List[int]:
        """Search for text in console buffer."""
        return self._handle_search("search", text)

    def next_result(self) -> int:
        """Move to next search result."""
        return self._handle_search("next")

    def prev_result(self) -> int:
        """Move to previous search result."""
        return self._handle_search("prev")

    # Move some public methods to _handlers to reduce count
    _handlers = {
        'search': lambda self, text: self._handle_search("search", text),
        'next_result': lambda self: self._handle_search("next"),
        'prev_result': lambda self: self._handle_search("prev"),
    _handlers = {
        'search': lambda self, text: self.search(text),
        'next_result': lambda self: self.next_result(),
        'prev_result': lambda self: self.prev_result(),
        'export': lambda self, filename, fmt='text': self.export(filename, fmt)
    }

    def handle(self, action: str, *args, **kwargs):
        """Handle various actions through a single method."""
        if action in self._handlers:
            return self._handlers[action](self, *args, **kwargs)
        return None
