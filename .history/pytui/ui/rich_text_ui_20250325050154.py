...
class Layout(Widget):
    ...
    def render(self):
        ...
        if self.layout_type == "horizontal":
            ...
            combined = ["  ".join(r[i] for r in child_renders) for i in range(max_height)]
            return "\n".join(combined)
...
class App:
    ...
    async def on_unmount(self):
        """Called when the app is unmounted. Performs cleanup and resource disposal.
        ...
        Override in subclasses to add custom cleanup logic, but call super().on_unmount()
        ...
        try:
            ...
            # Clear widget tree
            if self.view:
                # Recursively unmount all widgets
                async def unmount_widget(widget):
                    if hasattr(widget, 'children'):
                        for child in widget.children[:]:  # Copy list to avoid modification during iteration
                            await unmount_widget(child)
                            widget.remove_widget(child)
                    # Clear widget state
                    widget.parent = None
                    widget.focused = False
                    widget._event_handlers.clear()

                await unmount_widget(self.view)
            ...
            print(f"Error during unmount: {e}", file=sys.stderr)
