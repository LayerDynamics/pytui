...
from .rich_text_ui.progress import ProgressBarWidget, AnimatedSpinnerWidget, SpinnerWidget
...
class VariableInspectorWidget(Widget):
    ...
    def _format_value(self, value: Any, depth: int = 0) -> str:
        ...
        if isinstance(value, (list, tuple)):
            ...
            sep = ", "
            return f"[{sep.join(items)}]" if isinstance(value, list) else f"({sep.join(items)})"
    ...
    def render(self) -> str:
        ...
        if not variables:
            return Panel(
                f"No variables match filter: '{self._filter_text}'", 
                title=self.title
            ).render()
...
class KeyBindingsWidget(Widget):
    ...
    def render(self) -> str:
        ...
        if not self.bindings:
            return Panel("No key bindings defined", title=self.title, 
                         border_style=self.border_style).render()
