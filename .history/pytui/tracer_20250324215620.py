...
def trace_function(frame, event, arg):
    ...
    # Get collector first to avoid redundant checks
    ...
    # Handle internal files consistently
    if is_internal:
        # For call events in internal files, return trace_function to continue tracing
        if event == "call":
            return trace_function
        # For other events in internal files, return None
        return None
        
    # Process external file events
    if event == "call":
        collector.add_call(function_name, filename, frame.f_lineno, {})
        return trace_function
    elif event == "return":
        collector.add_return(function_name, arg)
        return None
    elif event == "exception":
        exc = arg[1] if isinstance(arg, tuple) and len(arg) >= 2 else None
