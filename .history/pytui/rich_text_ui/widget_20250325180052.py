...
class Widget:
    ...
    async def refresh(self):
        ...
        # Instead of directly rendering to stdout, we'll just prepare the content
        # and let the parent application handle the actual display
        try:
            # Signal any parent widgets that they need to refresh
            if self.parent:
                await self.parent.refresh()
            return True
        except Exception as e:
            # Log error but don't crash
            print(f"Error refreshing widget: {e}", file=sys.stderr)
            return False
