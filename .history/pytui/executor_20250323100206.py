...
class ScriptExecutor:
    ...
    def start(self):
        ...
        bootstrap_code = (
            ...
            "    from pytui.tracer import install_trace, _write_trace_event\n"
            "    from pytui.collector import get_collector\n"
            "    # Get the trace path from environment\n"
            "    trace_path = os.environ.get('PYTUI_TRACE_PATH')\n"
            "    if not trace_path:\n"
            "        print('ERROR: No trace path specified', file=sys.stderr)\n"
            "        return None\n"
            "    # Initialize collector and install tracer\n"
            "    collector = get_collector()\n"
            "    install_trace(collector, trace_path)\n"
            "    # Write test event to verify trace file is working\n"
            "    _write_trace_event('test', {'message': 'Trace initialized'})\n"
            "    return collector\n"
            "\n"
