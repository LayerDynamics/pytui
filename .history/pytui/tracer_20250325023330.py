...
def _should_skip_file(filename: str) -> bool:
    ...
    # Skip internal pytui files but not test files
    normalized_path = filename.replace("\\", "/")
    is_pytui = "pytui" in normalized_path.split("/")
    is_test = "tests" in normalized_path.split("/")
    return is_pytui and not is_test
    ...
    try:
        if event == "call":
            ...
            if not is_internal:
                ...
                collector.add_call(
                    function_name, filename, frame.f_lineno, args_dict, call_id=call_id
                )

        elif event == "return":
            call_id = _call_stack.pop() if _call_stack else 0
            if not is_internal:
                collector.add_return(function_name, str(arg), call_id=call_id)

        elif event == "exception":
            if not is_internal:
                _, exc_value, tb = arg
                collector.add_exception(exc_value, traceback=tb)

    except Exception as e:
        print(f"Error in trace_function: {e}")

    # Always return trace_function for call events to maintain the trace chain
