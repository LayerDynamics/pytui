...
def trace_function(frame, event, arg):
    ...
    collector = _collector or get_collector()  # Try global first, then getter
    if not collector:
        return None
    ...
    # For internal files, return None for all events
    if is_internal:
        return None

    try:
        # Handle each event type using event objects
        if event == "call":
            ...
            exception_event = ExceptionEvent(exc)
            collector.add_exception(exception_event.exception)
            return None
...
def install_trace(collector=None, trace_path=None):
    ...
    if collector is None:
        collector = DataCollector()  # Use imported DataCollector directly
