...
# Add FileState for managing trace file state
class FileState:
    """State for trace file handling."""
    def __init__(self):
        self.current_trace_file = None
        self.trace_path = None

    def open_trace_file(self, path: Optional[Union[str, Path]] = None):
        """Open a new trace file."""
        if path:
            self.trace_path = Path(path)
            self.current_trace_file = open(self.trace_path, 'w', encoding='utf-8')
...
_call_stack: List[int] = []
_call_id = 0
_state = FileState()

def _safe_str(value: Any) -> str:
    """Safely convert a value to string, handling Path objects specially."""
...
def _convert_args(frame: FrameType) -> Dict[str, str]:
    """Convert frame locals to string representation."""
    args = {}
    if frame.f_locals:
        for name, value in frame.f_locals.items():
            if name != 'self':  # Skip self reference
                try:
                    args[name] = _safe_str(value)
                except Exception:
                    args[name] = "<conversion failed>"
    return args

def _should_skip_file(filename: str) -> bool:
    """Check if file should be skipped for tracing."""
    parts = filename.split(os.sep)
    # Skip internal pytui files except executor.py
    return ("pytui" in parts and 
            "tracer.py" in filename or 
            "collector.py" in filename)

def _get_next_call_id() -> int:
    """Get next call ID."""
    global _call_id
    _call_id += 1
    return _call_id

def _handle_call_event(frame: FrameType, func_name: str, collector: DataCollector) -> Any:
    """Handle function call event."""
    if _should_skip_file(frame.f_code.co_filename):
        return None
...
def _handle_return_event(arg: Any, func_name: str, collector: DataCollector) -> None:
    """Handle function return event."""
    if _call_stack:
        call_id = _call_stack.pop()
        collector.add_return(func_name, _safe_str(arg), call_id)
                ...
                try:
                    ...
                    if isinstance(value, (int, float, bool, str, type(None))):
                        ...
                        if length <= 5:
                            ...
                            sample = sample[:-1] + ", ...]" if isinstance(value, list) else sample[:-1] + ", ...)"
                        ...
                        if length <= 3:
                            ...
                            sample = "{" + ", ".join(f"{repr(k)}: {repr(value[k])}" for k in keys) + ", ...}"
