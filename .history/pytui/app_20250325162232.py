...
# Local imports
from textual.app import App  # Update import path
from .ui.compat import App, USING_FALLBACKS
from .executor import ScriptExecutor
from .ui.panels import OutputPanel, CallGraphPanel, ExceptionPanel
from .widgets import (
    StatusBar,
...
class PyTUIApp(App):
    ...
    async def action_toggle_pause(self):
        ...
        if self.is_paused:
            self.executor.pause()
            await maybe_await(self.output_panel.add_message("Execution updates paused"))
        else:
            self.executor.resume()
            await maybe_await(
                self.output_panel.add_message("Execution updates resumed")
            )
    ...
    async def action_restart(self):
        ...
        if not self.executor:
            return
        await maybe_await(self.output_panel.clear())
        await maybe_await(self.call_graph_panel.clear())
        await maybe_await(self.exception_panel.clear())
        self.executor.restart()
        await maybe_await(self.output_panel.add_message("Restarting execution..."))

    async def action_search(self):
        """Activate search in the output panel."""
        await maybe_await(self.output_panel.focus())
        await maybe_await(self.output_panel.action_search())
    ...
    async def action_toggle_vars(self):
        ...
        if hasattr(self.call_graph_panel, "toggle_var_display"):
            await maybe_await(self.call_graph_panel.toggle_var_display())
    ...
    async def action_toggle_metrics(self):
        ...
        if self._metrics_visible:
            ...
            await maybe_await(
                self.view.dock(self.metrics_widget, edge="right", size=30)
            )
        else:
            try:
                self.view.remove_widget(self.metrics_widget)
