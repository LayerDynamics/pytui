...
async def test_complex_arguments_tracing(complex_args_script):
    ...
    from pytui.executor import ScriptExecutor  # import inside test to prevent toplevel warnings
    executor = None
    try:
        ...
        returns = [ret for ret in executor.collector.returns
                   if ret.function_name == "function_with_complex_args"]
        assert returns, "Complex function return was not traced"
        assert "'Return value'" in returns[0].return_value, "Return value not captured correctly"
    finally:
        if executor:
...
async def test_exception_tracing(exception_script):
    ...
    try:
        ...
        for phrase in ["Caught division by zero", "Caught KeyError", 
                       "Caught ValueError", "Caught top-level exception", 
...
async def test_module_import_tracing(module_import_script):
    ...
    try:
        ...
        calls = [call for call in executor.collector.calls if call.function_name == "use_imports"]
        assert calls, "use_imports function was not traced"
        outputs = " ".join(line.content for line in executor.collector.output)
        for phrase in ["Current directory", "Random number", "Current time", "Result: 3.14", "Expected import error"]:
...
def test_concurrent_tracing():
    ...
    for th in threads:
        th.join()
    outputs = [line for line in collector.output if line.content.startswith("Thread ") and "running" in line.content]
