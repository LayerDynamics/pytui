...
def controlled_executor():
    ...
    def add_exception(exc_type, message, traceback=None):
        # Create an exception instance with the correct type
        if exc_type == "ValueError":
            ...
            # Don't try to set __name__ as it will fail
...
async def test_app_executor_integration(debug_app, controlled_executor):
    ...
    await asyncio.sleep(0.2)
    assert (
        debug_app.output_panel.add_output.call_count == output_call_count
...
async def test_error_handling_during_event_processing(debug_app, controlled_executor):
    ...
    debug_app.output_panel.add_output.side_effect = ValueError(
        "Test output panel error"
    )
    debug_app.call_graph_panel.add_call.side_effect = KeyError(
        "Test call graph panel error"
    )
    debug_app.exception_panel.add_exception.side_effect = RuntimeError(
        "Test exception panel error"
    )
...
async def test_high_volume_event_processing(debug_app, controlled_executor):
    ...
    assert (
        debug_app.output_panel.add_output.call_count >= 500
    ), "Not all output events were processed"
    assert (
        debug_app.call_graph_panel.add_call.call_count >= 100
    ), "Not all call events were processed"
    assert (
        debug_app.call_graph_panel.add_return.call_count >= 100
