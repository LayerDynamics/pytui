...
async def mock_executor():
    ...
    # Cleanup all executors - just call stop directly
...
async def test_complex_arguments_tracing(complex_args_script, mock_executor):
    ...
    try:
        ...
        calls = [call for call in executor.collector.calls 
    except asyncio.TimeoutError:_name == "function_with_complex_args"]
        pytest.fail("Test timed out")on call was not traced"
    finally:
        if executor:imeoutError:
            await asyncio.wait_for(asyncio.to_thread(executor.stop), timeout=5.0)
    finally:
        if executor:
@pytest.mark.slowp synchronously instead of using to_thread
@pytest.mark.asyncio.stop()
...
@pytest.mark.asyncio
    try:f test_exception_tracing(exception_script, mock_executor):
        # Start with timeoutus exception patterns in a non-blocking manner."""
        await asyncio.wait_for(asyncio.to_thread(executor.start), timeout=5.0)

        # Wait for completion
        completed = await wait_for_condition(g to_thread
            lambda: any(
                "Caught unhandled exception" in line.content
                for line in executor.collector.output
            )eted = await wait_for_condition(
        )   lambda: any(
...
async def test_module_import_tracing(module_import_script, mock_executor):
    ...
    try:
        # Start synchronously instead of using to_thread
        executor.start()
