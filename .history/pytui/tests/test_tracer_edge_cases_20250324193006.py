...
async def test_exception_tracing(exception_script, mock_executor):
    ...
    try:
        ...
        # Wait for completion
        completed = await wait_for_condition(
            lambda: any("Caught unhandled exception" in line.content 
                      for line in executor.collector.output)
        )
...
async def test_module_import_tracing(module_import_script, mock_executor):
    ...
    try:
        ...
        # Wait for completion
        completed = await wait_for_condition(
            lambda: any("Result: 3.14" in line.content 
                      for line in executor.collector.output)
        )
...
async def test_concurrent_tracing():
    ...
    tasks = [traced_function(i) for i in range(5)]
    
    try:
        # Use wait instead of gather for better timeout handling
        ...
        assert len(done) == 5, f"Not all tasks completed: {len(done)}/5"
        
        # Verify outputs without blocking
        outputs = [line for line in collector.output
                  if line.content.startswith("Thread ") and "running" in line.content]
        assert len(outputs) == 5, "Not all outputs were captured"
        
    except asyncio.TimeoutError:
        pytest.fail("Concurrent tracing test timed out")
    finally:
