...
def minimal_script():
    ...
    with tempfile.NamedTemporaryFile(suffix=".py", delete=False, mode="w") as f:
        f.write(
            """
script_path = "temp_dummy_script.py"
def test_function(x, y):
    print(f"Processing {x}, {y}")
    return x + y

result = test_function(21, 21)
...
def test_trace_function_output():
    ...
    install_trace(collector)  # Ensure tracing is enabled
    ...
    frame_info = type('FrameInfo', (), {
        'f_code': type('CodeInfo', (), {
    ...
    assert any(
        call.function_name == 'function_to_trace' 
