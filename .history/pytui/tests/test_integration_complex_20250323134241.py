...
from pytui.collector import DataCollector, ReturnEvent
...
def mock_collector():
    ...
    async def add_test_function_call(func_name="test_func", filename="test.py", line_no=1, args=None):
        await asyncio.sleep(0.01)  # Small delay for async simulation
        if args is None:
            args = {"arg1": "value1"}
...
def controlled_executor():
    ...
    async def add_test_call(func_name="test_func", filename="test.py", line_no=1, args=None):
    ...
    async def add_test_exception(exc_type="ValueError", message="Test error", _traceback=None):
...
async def test_call_processing(debug_app, controlled_executor):
    ...
    await controlled_executor.add_test_call("test_function", "test.py", 42, {"arg": "value"})
    await asyncio.sleep(0.1)  # Give time for event to process
...
async def test_return_processing(debug_app, controlled_executor):
    ...
    # Add test function return
    await controlled_executor.add_test_return("test_function", "result_value")
    await asyncio.sleep(0.1)  # Give time for event to process
...
async def test_collapse_functionality(debug_app, controlled_executor):
    ...
    debug_app.collapsed_panels = {"call_graph": False, "output": False, "exception": False}
...
async def test_metrics_toggle(debug_app, controlled_executor):
    ...
    debug_app.view.dock.assert_called_with(debug_app.metrics_widget, edge="right", size=30)
