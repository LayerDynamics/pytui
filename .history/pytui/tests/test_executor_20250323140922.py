...
def test_execution(sample_script):
    ...
    try:
        ...
        while time.time() - start_time < max_wait and not output_found:
            if any(
                "Hello stdout" in line.content for line in executor.collector.output
            ):
        ...
        assert (
            "Hello stdout" in output_text
        ), f"Expected 'Hello stdout' not found in: {outputs[:5]}"

        # Check for stderr output
        stderr_found = any(
            "Hello stderr" in line.content for line in executor.collector.output
        )
        ...
        if len(executor.collector.calls) > 0:
            ...
            if function1_debug_output:
                print(
                    "Debug: Found function1 through debug output, but not traced calls"
                )
...
def test_stop():
    ...
    with patch("pytui.executor.terminate_process_tree") as mock_term, patch(
        "pytui.executor.kill_process_tree"
    ) as mock_kill:

        # Stop the execution
        executor.stop()

        # Check process termination was attempted
...
def test_restart(sample_script):
    ...
    try:
        ...
        assert "Hello stdout" in " ".join(
            initial_outputs
        ), f"Initial output not found in: {initial_outputs}"
        ...
        assert "Hello stdout" in " ".join(
            new_outputs
        ), f"Output after restart not found in: {new_outputs}"
    finally:
        # Ensure cleanup
        if executor:
            executor.stop()
