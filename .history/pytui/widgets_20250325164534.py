...
# Import from our rich_text_ui implementation instead of reimplementing
...
class BaseStatic(Static):
    """Base Static widget with required methods for async compatibility."""
    
    async def refresh(self):
        """Refresh the widget display asynchronously."""
        return await self.update(self.render())
    
    async def update(self, content):
        """Update widget content asynchronously."""
...
class SearchBar(BaseStatic):
    ...
    # Properties to access configuration elements
    @property
...
# Re-export the progress widgets from rich_text_ui
# No need to reimplement ProgressBarWidget - just use what's in rich_text_ui
# No need to reimplement AnimatedSpinnerWidget - just use what's in rich_text_ui

# Add async methods to the imported classes to ensure compatibility
...
async def async_update(self, completed, description=None):
    """Async update method for progress widgets."""
    self.completed = min(completed, self.total)
    if description:
        self.description = description
    return self.refresh()

# Add async methods to the imported classes
ProgressBarWidget.async_update = async_update
ProgressBarWidget.async_refresh = async_refresh
AnimatedSpinnerWidget.async_start = async_start
AnimatedSpinnerWidget.async_stop = async_stop
AnimatedSpinnerWidget.async_refresh = async_refresh
