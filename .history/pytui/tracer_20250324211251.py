...
def _should_skip_file(filename: str) -> bool:
    ...
    if "pytui/tracer.py" in filename.replace("\\", "/"):
        return True
    # Skip internal pytui modules
    return "pytui/tracer.py" in filename or "pytui/collector.py" in filename
...
def trace_function(frame, event, arg):
    ...
    # Special case for internal files
    if _should_skip_file(filename):
        # For calls to internal files, return the trace function to continue tracing
        # For other events in internal files, return None to skip them
        if event == "call":
    ...
    if event == "call":
        ...
        if _collector:
            _collector.add_call(func_name, filename, frame.f_lineno, args)
        return trace_function
    elif event == "return":
        func_name = frame.f_code.co_name
        try:
        ...
        if _collector:
            _collector.add_return(func_name, return_value_str)
        return trace_function
    elif event == "exception":
        exc_info = arg  # This should be a tuple (exc_type, exc_value, traceback)
        exception_type, exception_value, _ = exc_info
        if _collector:
            _collector.add_exception(exception_value)
        return trace_function
    
    # For other events like 'line', 'c_call', 'c_return', 'c_exception'
    return trace_function
