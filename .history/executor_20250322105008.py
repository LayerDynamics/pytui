    ...
    def _read_trace_data(self, trace_path):
        ...
        try:
            ...
            if not os.path.exists(trace_path):
                print(f"Debug: Trace file does not exist")
                return
            ...
            if file_size == 0:
                print("Debug: Trace file is empty")
                return

            # Read the entire file content at once to analyze
            with open(trace_path, "r") as f:
                content = f.read()

            # Check if we have valid content
            if not content.strip():
                print("Debug: Trace file has no content after stripping whitespace")
                return

            # Try processing line by line
