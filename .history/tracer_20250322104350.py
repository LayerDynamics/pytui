...
def trace_function(frame, event, arg):
    ...
    # Fix: Skip internal modules with safer string condition
    # Avoid checking tracer.py when testing specific modules
    if 'pytui' in str(filename) and ('tracer.py' in str(filename) or 'collector.py' in str(filename)):
        return None

    # Handle different event types
    if event == 'call':
        # Improved debug output for important functions
        ...
        # Send to parent process via IPC if available - with better error handling
        if _trace_file:
            try:
                event_data = {
                    'type': 'call',
                ...
                _trace_file.write(json_data + '\n')
                _trace_file.flush()
                
                # Extra debug for function1
                if func_name == 'function1':
                    print(f"Debug: Wrote function1 call to trace file")
            except Exception as e:
                print(f"Error writing trace data: {e}")
